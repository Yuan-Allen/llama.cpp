...
struct simple_model {
    ...
    // the context to define the tensor information (dimensions, size, memory
    // address)
    struct ggml_context *ctx;
};

// initialize the tensors of the model in this case two matrices 2x2
void load_model(simple_model &model, float *a, float *b, int rows_A, int cols_A,
...
#ifdef GGML_USE_TOPSCC
    fprintf(stderr, "%s: using TOPSCC backend\n", __func__);
    model.backend = ggml_backend_topscc_init(0); // init device 0
    if (!model.backend) {
        fprintf(stderr, "%s: ggml_backend_topscc_init() failed\n", __func__);
    }
#endif
    ...
    // create a backend buffer (backend memory) and alloc the tensors from the
    // context
...
struct ggml_cgraph *build_graph(const simple_model &model) {
    static size_t buf_size = ggml_tensor_overhead() * GGML_DEFAULT_GRAPH_SIZE +
                             ggml_graph_overhead();
...
struct ggml_tensor *compute(const simple_model &model, ggml_gallocr_t allocr) {
    // reset the allocator to free all the memory allocated during the previous
    // inference
    ...
    int n_threads =
        1; // number of threads to perform some operations with multi-threading
